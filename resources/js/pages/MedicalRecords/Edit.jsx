...
export default function Edit() {
    const { record, patients, errors } = usePage().props;
    const [form, setForm] = useState(record);

    const handleChange = (e) => setForm({...form, [e.target.name]: e.target.value});
    const handleSubmit = (e) => { e.preventDefault(); Inertia.post(route('medical_records.update', record.id), form); };

    return (
        <div>
            <h1 className="text-2xl font-bold mb-4">تعديل السجل الطبي</h1>
